...
def screen_registration_clients():
    def save_client(e):
        if (
            ...
            or not input_sobrenome.value
            or not input_cpf.value
            or not input_telefone.value
            or not input_endereco.value
            or not input_bairro.value
            or not input_cidade.value
            or not input_estado.value
            or not input_marca.value
            or not input_modelo.value
            or not input_ano.value
            or not input_placa.value
        ):
            ...
            salvar_cliente = clientsavedb.save_client_db(
                nome,
                sobrenome,
                cpf,
                telefone,
    ...
    def formatar_celular(valor):
        numeros = re.sub(r'\D', '', valor)[
            :11
        ]  # Remove não dígitos e limita a 11
    ...
    input_nome = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.PERSON
    )
    input_sobrenome = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.PERSON_OUTLINE
    )
    input_cpf = inputs.criar_input(
        on_change=ao_digitar_cpf, prefix_icon=ft.Icons.BADGE
    )
    input_telefone = inputs.criar_input(
        on_change=ao_digitar_telefone, prefix_icon=ft.Icons.PHONE
    )
    input_endereco = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.HOME
    )
    input_bairro = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.LOCATION_CITY
    )
    input_cidade = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.LOCATION_ON
    )
    input_estado = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.PUBLIC
    )
    input_marca = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.FACTORY
    )
    input_modelo = inputs.criar_input(
        on_change=inicial_maiuscula, prefix_icon=ft.Icons.DESIGN_SERVICES
    )
    input_ano = inputs.criar_input(
        on_change=ao_digitar_ano, prefix_icon=ft.Icons.CALENDAR_MONTH
    )
    input_placa = inputs.criar_input(
        on_change=ao_digitar_placa, prefix_icon=ft.Icons.CONFIRMATION_NUMBER
    )

    linha_nome = ft.Row(
        controls=[container_text_nome, input_nome],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_sobrenome = ft.Row(
        controls=[container_text_sobrenome, input_sobrenome],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_cpf = ft.Row(
        controls=[container_text_cpf, input_cpf],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_telefone = ft.Row(
        controls=[container_text_telefone, input_telefone],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_endereco = ft.Row(
        controls=[container_text_endereco, input_endereco],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_bairro = ft.Row(
        controls=[container_text_bairro, input_bairro],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_cidade = ft.Row(
        controls=[container_text_cidade, input_cidade],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_estado = ft.Row(
        controls=[container_text_estado, input_estado],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_marca = ft.Row(
        controls=[container_text_marca, input_marca],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_modelo = ft.Row(
        controls=[container_text_modelo, input_modelo],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_ano = ft.Row(
        controls=[container_text_ano, input_ano],
        alignment=ft.MainAxisAlignment.CENTER,
    )
    linha_placa = ft.Row(
        controls=[container_text_placa, input_placa],
        alignment=ft.MainAxisAlignment.CENTER,
    )
            ...
            controls=[
                ft.Row(
                    controls=[
                        ...
                        ft.Text('Cliente', size=20, weight=ft.FontWeight.BOLD),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
                linha_nome,
            ...
            controls=[
                ft.Row(
                    controls=[
                        ...
                        ft.Text(
                            'Localização', size=20, weight=ft.FontWeight.BOLD
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
            ...
            controls=[
                ft.Row(
                    controls=[
                        ...
                        ft.Text(
                            'Motocicleta', size=20, weight=ft.FontWeight.BOLD
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
    ...
    linha_containers = ft.Row(
        controls=[container_dados_pessoas, container_motocicleta]
    )

    tela_cadastro_clientes = ft.Container(
        content=ft.Column(
            controls=[linha_containers, container_localizacao], expand=True
        ),
        expand=True,
    )
